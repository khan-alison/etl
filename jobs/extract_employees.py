...
class JobExecutor(BaseExecutor):
    ...
    def _join_data(self, df_employee_filtered, df_rate):
        df_rate.printSchema()
        df_joined = df_employee_filtered.alias('r').join(df_rate.alias('l'), how='left', on=(
                F.col("r.BusinessEntityId") == F.col("l.BusinessEntityId"))).withColumn("partition_date", F.to_date(
            F.col("r.ModifiedDate"), DATE_FORMAT)).drop("r.ModifiedDate")
...
if __name__ == "__main__":
    ...
    configurations = {
        "appName": "PySpark learning - MariaDB",
        "master": "local",
